...
class _FourierLayer(nn.Model):
    ...
    def forward(self, points):
        ...
        padding = torch.zeros(2*len(self.mode_num), device=points.device, 
                                dtype=torch.int32)
        padding[1::2] = torch.flip((self.mode_num - torch.tensor(points.shape[2:])), 
                                    dims=(0,))
        ...
        ifft = torch.fft.ifftn(
                ...
                dim=len(self.mode_num), norm="ortho")
...
class FNO(Model):
    ...
    def __init__(self, input_space, output_space, 
                 upscale_size, fourier_layers, fourier_modes, 
                 activations, xavier_gains):
