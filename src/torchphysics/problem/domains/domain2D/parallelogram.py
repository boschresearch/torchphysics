...
class Parallelogram(Domain):
    ...
    def bounding_box(self, params=Points.empty(), device="cpu"):
        origin, corner_1, corner_2, _, _ = self._construct_parallelogram(
    ...
    def _contains(self, points, params=Points.empty()):
        origin, _, _, dir_1, dir_2 = self._construct_parallelogram(
            points.join(params), points.device
        )
...
class ParallelogramBoundary(BoundaryDomain):
    ...
    def _contains(self, points, params=Points.empty()):
        origin, _, _, dir_1, dir_2 = self.domain._construct_parallelogram(
            points.join(params), points.device
        )
        points = points[:, list(self.space.keys())].as_tensor
        points -= origin
    ...
    def _get_volume(self, params=Points.empty(), device="cpu"):
        _, _, _, dir_1, dir_2 = self.domain._construct_parallelogram(
            params, device=device
        )
    ...
    def sample_random_uniform(
        ...
        self._transform_interval_to_boundary(
            dir_1, dir_2, side_1, side_2, points, bound_location
        )
        points += origin[:, None, :]
        return Points(points.reshape(-1, self.space.dim), self.space)
    ...
    def _transform_interval_to_boundary(
        self, dir_1, dir_2, side_1, side_2, points, bound_location
    ):
    ...
    def normal(self, points, params=Points.empty(), device="cpu"):
        ...
        self._add_local_normal_vector(
            normals, bary_x, bary_y, normal_dir_1, normal_dir_2, 0.0
        )
        self._add_local_normal_vector(
    ...
    def _add_local_normal_vector(
        self, normals, bary_x, bary_y, normal_dir_1, normal_dir_2, i
    ):
