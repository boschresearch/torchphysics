...
def FDM(domain_dic, step_width_dic, time_interval, variable_list, inital_condition):
    solution = []
    time_domains = []
    ...
    for k in range(len(variable_list)):
        ...
        u[0,:,:] = _set_inital_condition(u[0,:,:], domain, time[0], inital_condition)
        for i in range(1, len(time[0])):
            u[i,:,:] = do_timestep(u[i-1,:,:], domain, time, step_width_dic, variable_list[k])
        solution.append(u)
...
def _set_inital_condition(u, domain, time, inital_condition):
    u0 = u.copy()
    dic = {'t' : time}
    for i in range(len(domain[0])):
        for j in range(len(domain[1])):
            dic['x'] = np.array([domain[0][i], domain[1][j]]).reshape(-1,2)
            u0[i,j] = inital_condition(dic)
    return u0
